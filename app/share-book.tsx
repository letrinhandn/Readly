...
export default function ShareBookScreen() {
  ...
  const bookStats = {
    totalMinutes: book.totalMinutesRead || 0,
    totalPages: book.currentPage,
    sessionCount: book.sessionCount || 1,
  };
  ...
  const handleShare = async () => {
    try {
      ...
      if (Platform.OS === 'web') {
        ...
        a.href = uri;
        a.download = `readly-book-${Date.now()}.png`;
        a.click();
      } else {
  ...
  const handleThemeSelect = (themeId: ShareThemeType) => {
    ...
    if (theme.isPremium && !isPremiumUser) {
      Alert.alert('Premium Theme', 'This theme is available for premium users only. Upgrade to unlock all themes!');
      return;
    }
    if (Platform.OS !== 'web') {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.header}>
        <Text style={[styles.headerTitle, { color: colors.text }]}>Share Book Progress</Text>
        <TouchableOpacity
      ...
      >
        <View style={styles.cardContainer} collapsable={false} ref={cardRef}>
          <ShareBookCard book={book} stats={bookStats} theme={selectedTheme} />
        </View>
                ...
                >
                  ...
                  {isLocked && (
                    <View style={styles.lockBadge}>
                      <Crown size={14} color={colors.warning} strokeWidth={2.5} fill={colors.warning} />
                    </View>
                  )}
                  {isSelected && !isLocked && (
                    <View style={[styles.selectedBadge, { backgroundColor: colors.primary }]}>
